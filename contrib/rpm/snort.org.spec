# $Id: snort.org.spec,v 1.8.2.2 2004/08/09 15:03:49 jhewlett Exp $
...
plugin architecture. Snort 2.1.0 adds regular expressions to the Snort
rules language, allowing almost unlimited flexibility in describing
packets. Snort has a real-time alerting capability as well, incorporating
alerting mechanisms for syslog, a user specified file, a UNIX socket, and
others.
...
# Fix a double path in the signature dir, if still bad, and move signature
# documentation to a more prominent place (not buried under doc).
	mv doc/signatures/signatures .
	rmdir doc/signatures
elif [ -d doc/signatures ]; then
# Else, move signature documentation to a more prominent place (not buried
# under doc).
	mv doc/signatures .
fi
...
InstallSnort() {
   ...
   if [ "$1" = "plain" ]; then
	...
	install -p -m 0755 contrib/rpm/snortd $RPM_BUILD_ROOT/etc/init.d
